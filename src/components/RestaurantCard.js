const RestaurantCard = ({name,cuisines,slaString,cloudinaryImageId,costForTwoString,avgRating,aggregatedDiscountInfo})=>{
  // card element
    return (<div className="card w-80 bg-base-100 my-5 transition duration-700 ease-in-out transform hover:-translate-y-1 hover:scale-105 hover:border-2  p-4 m-4 ">
    ...
    <div className="flex-col justify-center items-center">
      ...
      {/* Juaggad */}
      {aggregatedDiscountInfo?.shortDescriptionList[0]?.meta!=undefined ?(<div><p className="text-clip text-gray-300 font-extralight">___________________________________________</p>  <p className="text-orange-900  font-normal">{aggregatedDiscountInfo?.shortDescriptionList[0]?.meta}</p></div>
):(<></>) }
